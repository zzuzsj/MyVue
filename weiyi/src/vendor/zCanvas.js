...
ZCanvas.prototype = {
    ...
    animate: function () {
        ...
        for (var i = 0; i < this._canvasNodeArray.length; i++) {
            ...
            if (node._animating) {
                ...
                switch (node.nType) {
                    case 'pointline':
                        for (var i = 0; i < node.curveArray.length; i++) {
                            var _node = node.curveArray[i];
                            _node.__animateFrame(_node);
                        }
                        break;
                    default:
    ...
    deRenderPointLine: function (node, config, options, callback) {
        if (node.nType !== 'pointline' || node._nodeType !== 'node') {
            console.warn('Node must be a pointline node');
            return;
        }
        if (!node._inCanvas) {
            console.warn('Node is not added');
            return;
        }
        var curveArr = node.curveArray;
        if (!config || config.time === 0) {
            for (var i = 0; i < curveArr.length; i++) {
                var _node = curveArr[i];
                _node.curAttr = _node.attr;
            }
            ...
            var _time = Math.floor(time / curAttr.length);
            for (var i = 0; i < curveArr.length; i++) {
                config.time = _time;
                config.delay = delay + i * _time;
                if (i === (curveArr.length - 1)) {
                    _node.animate(config, options);
                } else {
                    _node.animate(config, options, function (callback) {
                        if (callback) {
                            callback();
                            node._rendered = false;
                            this._canvasNodeArray.splice(node._canvasIndex, 1);
    ...
    renderPointLine: function (node, config, options, callback) {
        if (node.nType !== 'pointline' || node._nodeType !== 'node') {
            console.warn('Node must be a pointline node');
            return;
        }
        ...
        if (!config || config.time === 0) {
            ...
            var time = config.time;
            var delay = config.delay;
            var _time = Math.floor(time / curveArr.length);
            node._animating = true;
            for (var i = 0; i < curveArr.length; i++) {
                var _node = curveArr[i];
                config.time = _time;
                config.delay = delay + i * _time;
                _node.animate(config, options, function () {
...
// ZCanvas.prototype.PointLine = function (options) {
//     var pointLine = new ZCanvas.PointLine(options, this._cacheContext);
//     var that = this;
//     for (var i = 0; i < (options.points - 1); i++) {
//         var x = Math.floor((options.pointX[i] + options.pointX[i + 1]) / 2);
//         var y;
//         if (i === 0) {
//             y = Math.floor(Math.random() * 500);
//         } else {
//             y = options.pointY[i] + (options.pointY[i] - pointLine.attr.ctrlPointY[i - 1]);
//         }
//         pointLine.attr.ctrlPointX.push(x);
//         pointLine.attr.ctrlPointY.push(y);
//         var curveLine = this.CurveLine({
//             curvePointX: [options.pointX[i], pointLine.attr.ctrlPointX[i], options.pointX[i + 1]],
//             curvePointY: [options.pointY[i], pointLine.attr.ctrlPointY[i], options.pointY[i + 1]],
//             curveQuar: 2,
//             lineLaunch: options.lineLaunch,
...
//         });
//         pointLine.curveArray.push(curveLine);
//     }
//     pointLine.animate = function (config, options, callback) {
//         if (!that._framing) {
//             that._framing = true;
//             that.animate();
//         }
...
// ZCanvas.PointLine = function (options, cacheCtx, CurveLine) {
//     this.__init(options, cacheCtx, CurveLine);
// }
// ZCanvas.PointLine.prototype = Object.create(ZCanvas.Node.prototype);    //Object.create():创建一个空对象，并且这个对象的原型指向它的参数  //这样子我们可以在访问Student.prototype的时候可以向上查找到Person.prototype,又可以在不影响Person的情况下，创建自己的方法
// ZCanvas.PointLine.prototype.constructor = ZCanvas.Node;
// ZCanvas.PointLine.prototype.__init = function (options, cacheCtx) {
//     ZCanvas.Node.call(this, options, cacheCtx);
//     this.nType = "pointline";
//     this.curveArray = [];
//     this.attr.ctrlPointX = [];
//     this.attr.ctrlPointY = [];
//     var that = this;

// };
// ZCanvas.PointLine.prototype.__draw = function () {
//     for (var i = 0; i < this.curveArray.length; i++) {
//         this.curveArray[i].__draw();
//     }
// }
